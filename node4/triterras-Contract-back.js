...
var Chaincode = class {
  ...
  */
  ...
  async addMember(stub, args) {
    ...
    newMember.checkId = stub.getMSPID();
 ...
 async assignUsers(stub, args) {
  ...
  let userBytes = await stub.getState(args[2]);
  console.info(userBytes)
  if (!userBytes || userBytes.length === 0) {
    throw new Error(`User with id ${args[2]} does not exist`);
  }

  let userBytes1 = await stub.getState(args[0]);
  console.info(userBytes1)
  if (!userBytes1 || userBytes1.length === 0) {
    throw new Error(`User2 of org ${args[3]} with id ${args[0]} does not exist`);
  }
...
* @param args[3]_is_recipientId - an Id given by buyer Bank
* @param args[4]_is_generateDate - the unique id to identify the order
*/
...
async generateLC(stub, args) {
  ...
  // fetch lc
  let lcBytes = stub.getState(args[2]);
