	...
	try {
		...
		if (args.length > 1 && args[0] !== 'range') {
			for (let i = 0; i < args.length; i++) {
				...
				let response_payloads = await channel.queryByChaincode(request, 120000);
				if (response_payloads) {
					res.push(`${args[i]}:`, response_payloads[0]);
				} else {
					logger.error('response_payloads is null');
					res.push('error:', `response_payloads is null ${args[i]}`);
				}

			}
			if (res) {
				// for (let i = 0; i < res; i++) {
				// 	logger.info(args[0]+' now has ' + res[i].toString('utf8') +
				// 		' after the move');
				// }
				return res;
			} else {
